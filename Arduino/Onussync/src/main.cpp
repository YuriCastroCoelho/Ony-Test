...
const float LIMITE_ALERTA_GRAMAS = 500.0; 
...
void dispararParaAPI(String tipoDeEvento, float pesoDaVez) {
  ...
  // Monta o JSON com a porcentagem calculada
  String json = "{\"tipo\": \"" + tipoDeEvento + "\", \"peso_porcentagem\": " + String(porcentagem) + "}";
  ...
  Serial.println("Valor enviado: " + String(porcentagem) + "%");

  // ATENÇÃO: Mudou de TCP para SSL para passar na segurança do Render
